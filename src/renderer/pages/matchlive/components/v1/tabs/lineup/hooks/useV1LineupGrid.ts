...
const createEmptyEvents = (): V1ViewPlayerEvents => ({
  subIn: false,
  yellow: false,
  red: false,
  goal: [],
...
const processTeamLineup = (
  ...
  const playersByGrid: V1ViewPlayer[][] = [];
...
const getCurrentPlayer = (player: V1ViewPlayer): V1ViewPlayer => {
  let current = player;
...
const isPlayerOnField = (
  ...
  lineupGrid: V1ViewPlayer[][]
): { found: boolean; rootPlayer?: V1ViewPlayer } => {
...
const updatePlayerGoal = (event: EventInfo, lineup: V1ViewLineup) => {
...
const updatePlayerCard = (event: EventInfo, lineup: V1ViewLineup) => {
...
const updatePlayerSubst = (event: EventInfo, lineup: V1ViewLineup) => {
  ...
  let subOutPlayer: V1ViewPlayer | null = null;
  ...
  if (substituteDetails && subOutPlayer) {
     const subInViewPlayer: V1ViewPlayer = {
       ...substituteDetails,
...
export const useV1LineupGrid = (
  ...
  const [processedHome, setProcessedHome] = useState<V1ViewLineup | null>(null);
  const [processedAway, setProcessedAway] = useState<V1ViewLineup | null>(null);
